...
function Terminal({ onCommand }){
  const [lines, setLines] = useState([`Lucidia shell — type 'help' to see commands.`]);
  ...
  return (
    <div style={{border:'1px solid var(--border)', borderRadius:'12px', background:'#0b1324', display:'flex', flexDirection:'column', height:260}}>
      ...
      <div style={{display:'flex', gap:8, padding:8, borderTop:'1px solid var(--border)'}}>
        ...
        <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={onKey} placeholder="Type a command…"
...
function Dashboard(){
  ...
  const [timeline, setTimeline] = useState(()=>[
    ...
    {id:2, kind:'revert', title:'Mistral agent reverted commit d1f6e52', time:'Today 10:29', desc:'Automatic rollback on failed checks.', actions:['Details']},
    {id:3, kind:'commit', title:'User committed changes', time:'Day', desc:'Added a simple print statement.', code:'print("Hello, world!")'},
    {id:4, kind:'build', title:'Build', time:'Today', desc:'Build pipeline running…', actions:['Open Logs']}
  ]);
  ...
  useEffect(()=>{
    const t = setInterval(()=>{ pushRand(cpu,setCPU); pushRand(mem,setMEM); pushRand(gpu,setGPU);
      ...
      if(Math.random() < .22){
        ...
        const title = kind==='build'? 'Build step finished' : kind==='commit'? 'Agent committed fix' : kind==='revert'? 'Auto-revert triggered' : 'Agent action';
  ...
  const onCommand = async (cmd)=>{
    ...
    switch(c){
      ...
      case 'run': runCode(); return ['executing current editor code…'];
      case 'build': setBuild(0); setTimeout(()=>setBuild(100), 5000); return ['build started (5s)…'];
      case 'commit': {
              ...
              <div style={{display:'grid', gap:10}}>
                {commits.map(c => (
                  <div key={c.id} style={{border:'1px solid var(--border)', padding:12, borderRadius:12, background:'#0b1324', display:'flex', justifyContent:'space-between'}}>
                    <div><strong>{c.id}</strong> — {c.msg}</div>
